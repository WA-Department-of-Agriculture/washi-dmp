...
Another example is the [readme.txt](Y:\NRAS\Soil_Health_Initiative\Sampling\2023_Sampling\LabData\Raw\readme.txt) in the `2023_Sampling` \> `LabData` \> `Raw` folder, which explains why there are two different versions of the lab results and where to find additional information.
...
Our [dataDictionary.xlsx](Y:\NRAS\Soil_Health_Initiative\Sampling_completeDataset\dataDictionary.xlsx) in the [\_completeDataset folder](Y:\NRAS\Soil_Health_Initiative\Sampling_completeDataset) contains two tabs (`labResults` and `sampleLocations`) that describe the attributes of each variable.
...
-   Do the lab results have accompanying metadata and management data?
