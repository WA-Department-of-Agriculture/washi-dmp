...
-   **snake_case**: all lowercase with underscores separating words. Only use for **column names** in spreadsheets and **code**, such as functions and variables in R. See [@sec-objects-functions] for example R errors when including hyphens in object names.
...
Naming conventions for column names in spreadsheets and objects in code differ from folders and files. The hyphens in kebab-case cause errors in R and SQL code. Additionally, hyphens, spaces, and other special characters are [invalid for ArcGIS table and field names](https://support.esri.com/en-us/knowledge-base/faq-what-characters-should-not-be-used-in-arcgis-for-fi-000005588).

Use **snake_case** for column names in spreadsheets and all **code objects** (R vectors, lists, dataframes, and functions).
...
Remember to not use special characters. Instead of `toc_%`, use `toc_percent`.
...
+-------------------------+-------------------+--------------------------------------+
| **Column Names & Code** | snake_case        | sample_id                            |
|                         |                   |                                      |
|                         |                   | clay_percent                         |
|                         |                   |                                      |
|                         |                   | pmn_lb_ac                            |
