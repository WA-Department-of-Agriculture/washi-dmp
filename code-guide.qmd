...
Let's first define these terms so we're on the same page. In this style guide, **variables** are column names in spreadsheets (that become column names in R dataframes), **objects** are all data structures in R and ArcGIS (vectors, lists, dataframes, fields, tables), and **functions** are self-contained modules of code that accomplish a specific task.
